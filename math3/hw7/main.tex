...
\section*{Exercise 27}
Lets create a set for the tickets
\begin{align*}
   T := B \uplus NCP \uplus TP \\
   B := \{ b \in \mathbb{N} \; | \; b < 160 \} \\
   NCP := \{ n \in \mathbb{N} \; | \; 160 \leq n < 198 \} \\
   TP := \{ t \in \mathbb{N} \; | \; 198 \leq t < 200 \}
\end{align*}
\(B\) is the set of blank tickets, \(NCP\) is the set of noncash prizes and \(TP\) is the set of top prizes and they are disjoint. \\

We choose the probability space
\begin{align*}
   \Omega &= \{ X \subseteq T \; | \; |X| = 10 \}, \\
   \mathcal{F} &= \text{Power set of } \Omega, \\
   P(A) &= U_\Omega
\end{align*}

We can choose uniform distribution because each ticket has equal chance of being selected. \\
The events are:
\begin{align*}
   A_1 &= \{ X \in \Omega \; | \; |X \cap TP| = 1 \text{ and } |X \cap NCP| \geq 2 \}, \\
   A_2 &= \{ X \in \Omega \; | \; |X \cap B| < 6 \}
\end{align*}

Here note that \(|\Omega| = \binom{200}{10}\) \\
Calculating the probabilities:
\begin{align*}
   \intertext{choose 1 top prize, choose 2 noncash prize, choose 7 either blank or noncash prizes to make it to 10 total}
   P(A_1) &= \frac{\binom{2}{1} \cdot \binom{38}{2} \cdot \binom{196}{7}}{|\Omega|} \\
   &= \frac{\binom{2}{1} \cdot \binom{38}{2} \cdot \binom{196}{7}}{\binom{200}{10}}, \\
   &= 53428/431233 \quad \text{link to the calculator: \url{https://www.wolframalpha.com/input?i=choose\%282\%2C1\%29+*+choose\%2838\%2C2\%29+*+choose\%28196\%2C7\%29+\%2F+choose\%28200\%2C10\%29}} \\
   \intertext{we separetely count when there is k blank tickets where k = 0, 1, 2, 3, 4, 5 and sum them up}
   P(A_2) &= \frac{\sum_{k=0}^{5} \binom{160}{k} \cdot \binom{40}{10 - k}}{|\Omega|} \\
   &= \frac{1}{\binom{200}{10}} \sum_{k=0}^{5} \binom{160}{k} \cdot \binom{40}{10 - k}.
\end{align*}
...
\subsection*{(i)}
A probability counting density must satisfy two properties:
\begin{itemize}
    \item \(p(k) \geq 0\) for all \(k\in \Omega\)
    \item \(\sum_{k \in \Omega}^{} p(k) = 1\)
\end{itemize}

\(p_1\) is not a probability counting density because it assigns negative probabilities to some outcomes. \\
\(p_2\) is not a probability counting density function because the sum exceeds 1.

\subsection*{(ii)}
We need to find \(\alpha, \beta \in \mathbb{R}\) such that \(\alpha p_1 + \beta p_2\) is a probability counting density. \\
So we need to satisfy the two properties:
\begin{itemize}
    \item \(\alpha p_1(k) + \beta p_2(k) \geq 0\) for all \(k\in \Omega\)
    \item \(\sum_{k \in \Omega}^{} \alpha p_1(k) + \beta p_2(k) = 1\)
\end{itemize}

From the second property we have:
\begin{align*}
   \sum_{k \in \Omega}^{} \alpha p_1(k) + \beta p_2(k) &= \alpha \sum_{k \in \Omega}^{} p_1(k) + \beta \sum_{k \in \Omega}^{} p_2(k) \\
   &= \alpha \cdot 1 + \beta \cdot 2 = \alpha + 2\beta = 1
\end{align*}

We are gonna replace \(\alpha\) with \(1 - 2\beta\) in the first property and write down all the inequalities and find one pair that satisfies all of them.
\begin{align*}
   (1 - 2\beta) p_1(k) + \beta p_2(k) &\geq 0 \\
   \intertext{for \(k = 1\) or \(k = 3\)}
   (1 - 2\beta) \left(-\frac{1}{3}\right) + \beta \left(\frac{1}{3}\right) &\geq 0 \\
   -\frac{1}{3} + \frac{2\beta}{3} + \frac{\beta}{3} &\geq 0 \\
   -1 + 3\beta &\geq 0 \\
   \beta &\geq \frac{1}{3} \\
   \intertext{for \(k = 2\) or \(k = 5\)}
   (1 - 2\beta) \left(-\frac{1}{6}\right) + \beta \left(\frac{1}{6}\right) &\geq 0 \\
   -\frac{1}{6} + \frac{2\beta}{6} + \frac{\beta}{6} &\geq 0 \\
   -1 + 3\beta &\geq 0 \\
   \beta &\geq \frac{1}{3} \\
   \intertext{for \(k = 4\) or \(k = 6\)}
   (1 - 2\beta) (1) + \beta \left(\frac{1}{2}\right) &\geq 0 \\
   1 - 2\beta + \frac{\beta}{2} &\geq 0 \\
   2 - 4\beta + \beta &\geq 0 \\
   2 - 3\beta &\geq 0 \\
   \beta &\leq \frac{2}{3} \\
\end{align*}

Choose \(\beta = \frac{1}{2}\) and \(\alpha = 1 - 2\beta = 0\). \\
So one possible linear combination is:
\begin{align*}
   0 \cdot p_1 + \frac{1}{2} \cdot p_2
\end{align*}

\subsection*{(iii)}
We need to find \(q \in (0, 1)\) such that \(p_q\) is a probability counting density. \\
We need to satisfy the two properties:
\begin{itemize}
    \item \(p_q(k) = q^k \geq 0\) for all \(k\in \Omega\)
    \item \(\sum_{k \in \Omega}^{} p_q(k) = \sum_{k \in \mathbb{N}}^{} q^k = 1\)
\end{itemize}

The first property is satisfied for all \(q \in (0, 1)\) since exponentials with positive bases are always positive. \\
For the second property we use the formula for infinite geometric series:
\begin{align*}
   \sum_{k=1}^{\infty} q^k &= \frac{q}{1 - q} = 1 \\
   q &= 1 - q \\
   2q &= 1 \\
   q &= \frac{1}{2}
\end{align*}

So the only \(q\) that makes \(p_q\) a probability counting density is \(q = \frac{1}{2}\).
